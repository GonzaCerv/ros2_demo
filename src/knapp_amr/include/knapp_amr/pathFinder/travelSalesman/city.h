...
class city {
   public:
    city(int idin, double xin, double yin, int posin, bool visitedin);
    city(city& source);
    ~city();
    double dist(city* city_in);          // calculates the Euclidean distance to another city
    ...
    void build_neighbor_list(deque<city*>& cities,
                             int num_cities);  // Builds neighbor list sorted by distance, taking a
                                               // deque of cities as input
