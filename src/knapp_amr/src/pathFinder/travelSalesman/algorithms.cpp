...
#include "knapp_amr/pathFinder/travelSalesman/algorithms.h"
...
// Constructor that takes a character string corresponding to an external filename and reads in
// cities from that file
tsp::tsp(const char* filename) { num_cities = read_file(filename); }

tsp::tsp(const std::vector<knapp_amr::Part>& parts) : original_parts{parts} {
    num_cities = read_vector(original_parts);
}
...
int tsp::read_vector(const std::vector<knapp_amr::Part>& parts) {
    original_list.clear();
    for (uint32_t index = 0; index < parts.size(); ++index) {
        original_list.emplace_back(new city(index, parts[index].x, parts[index].x, index, false));
    }
    return parts.size();
}
...
int tsp::brute_force_wrapper() {
    ...
    if (solution.empty()) nearest_neighbor_basic(0);
...
void tsp::brute_force(deque<city*>& best_path, int& min_distance, int cities_left) {
    ...
    for (int i = 0; !done && i < cities_left; ++i) {
        ...
        brute_force(best_path, min_distance,
                    cities_left - 1);  // shift ending position of rotation left 1 in each recursion
    }
...
std::vector<knapp_amr::Part> tsp::find_path() {
    std::vector<knapp_amr::Part> response;
    solution.clear();
...
// Generates nearest neighbor tour in solution from list of cities in original_list, starting at
// start_index.
    ...
    while (!done) {
        ...
        for (int i = 1; i < num_cities && !start_over; ++i) {
            for (int j = i + 1; j < num_cities - 1 && !start_over; ++j) {
                ...
                if (solution[i - 1]->dist(solution[j]) + solution[i]->dist(solution[j + 1]) <
...
int tsp::two_opt() {
    ...
    for (int i = 1; i < num_cities; ++i) {
        k = 1;
        fix_positions();
        while (k <= 5 && solution[i]->dist(solution[i]->get_neighbor(k)) <
                             solution[i - 1]->dist(solution[i])) {
...
std::vector<knapp_amr::Part> tsp::generate_solution() {
    std::vector<knapp_amr::Part> response;
    std::cout << "calling generate solution" << std::endl;
    for (uint32_t index = 0; index < solution.size(); ++index) {
        auto original_list_index = solution[index]->get_id();
        // When no part vector was provided, use the index as name for the Part.
        if (original_parts.empty()) {
            knapp_amr::Part part = {
                std::to_string(original_list_index),
                "",
                solution[index]->get_x(),
                solution[index]->get_y(),
            };
            response.emplace_back(part);
        } else {
            response.emplace_back(original_parts[original_list_index]);
        }
    }
    return response;
}
