...
#include "knapp_amr/pathFinder/travelSalesman/city.h"
...
#include "knapp_amr/pathFinder/travelSalesman/tree.h"
using namespace std;

const int LIST_SIZE = 5;\
const double DOUBLE_LIMIT = 0.001;

bool AreDoubleSame(double dFirstVal, double dSecondVal)
{
    return std::fabs(dFirstVal - dSecondVal) < DOUBLE_LIMIT;
}

// Constructor that takes all members as input
city::city(int idin, double xin, double yin, int posin, bool visitedin)
...
double city::dist(city* city_in) {
    ...
    return (sqrt(x2 + y2) + 0.5);  // Rounds down from 0.0 to .499_, rounds up from 0.5 to .99_
}

double city::get_x() { return x; }

double city::get_y() { return y; }

int city::get_id() { return id; }

int city::get_pos() { return pos; }

city*& city::get_neighbor(int index) { return neighbor_list[index]; }

int city::get_nl_size() { return LIST_SIZE; }

void city::set_pos(int pos_in) { pos = pos_in; }
...
void city::output_id(ostream& output) { output << id; }
...
int city::compare(const city* city_in) {
    if (city_in->id == id && AreDoubleSame(city_in->x, x) && AreDoubleSame(city_in->y, y)) {
        return 1;
    }
...
void city::build_neighbor_list(deque<city*>& cities, int num_cities) {
    tree sorted(this);  // Creates a tree with the current city as owner (to calculate distances for
                        // sorting)
...
bool city::nl_is_empty() { return neighbor_list.empty(); }

void city::push_to_list(city*& to_add) { neighbor_list.push_back(new city(*to_add)); }
...
int city::get_neighbor_pos(int index) { return neighbor_list[index]->pos; }
