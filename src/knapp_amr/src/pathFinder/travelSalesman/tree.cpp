...
// Return left pointer
tree_node*& tree_node::get_left() { return left; }

// Return right pointer
tree_node*& tree_node::get_right() { return right; }
...
// Return the item
city*& tree_node::get_city() { return tree_city; }
...
tree::~tree() { clear_tree(); }

// Copy constructor:
tree::tree(tree& source) { copy(source.root, root); }
...
void tree::add_to_tree(city*& to_add) { insert(root, to_add); }
...
void tree::display_detailed_tree() { traverse_and_display(root); }
...
void tree::clear_tree() { remove_all(root); }
